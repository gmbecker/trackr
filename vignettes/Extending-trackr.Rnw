...
lb = setRefClass("ListBackend", fields = list(dat = "list"))
ListBackend = function() lb$new()
@
...
All \rfun{instert_record} and \rfun{remove_record} methods must accept five and four arguments, respectively:
\begin{enumerate}
\item object - the document to be inserted or deleted within the datastore,
  or the id of the record to be deleted;
\item id - the unique id to associate with the added plot, or the id of the plot to remove (\emph{insert_record only};
...
<<>>=
setMethod("insert_record", c( target = "ListBackend"),
function(object, id, target, opts, verbose = FALSE) {
    target$dat[[id]] = object
    invisible(target)
})

setMethod("remove_record", c(object = "character", target = "ListBackend"), 
...
<<>>=
setMethod("trackr_write", c(target = "ListBackend"), 
...
<<>>=
setMethod("trackr_lookup", c("character", target = "ListBackend"),
...
setMethod("trackr_search", c(pattern = "character", target = "ListBackend"),
...
\rfun{makeFeatureSet} methods \rpgk{trackr} provides to perform
