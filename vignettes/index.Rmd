...
* From Github: `duncantl/CodeDepends`, `lawremi/rsolr`, `lawremi/restfulr`, `lawremi/rdocdb`
...
# Explore the functionality of trackr

We will first create =ObjFeatureSets= for each of the plots. This involves
generating the metadata and storing everything together in a form that =trackr=
will be able to use.
...
We will now =record= the plots. We need a TrackrDB to store them in. We will
use a simple, JSON based one which writes to a temporary file. Normally if we
wanted a JSON-based backend we could simply use the default, but because this is
a vignette run on the CRAN build systems we need to ensure no permanent
files are created.

```{r}
tdb = jsonTDB(tempfile(), img_dir = tempdir())
invisible(lapply(my.pfs, record, db = tdb))
...
```{r}
res <- findRecords("density", db = tdb)
ndoc(res)
res[1,"image.path"]
loadEnrichedPlot(file.path(vt_img_dir(tdb), res[1,"image.path"]))
